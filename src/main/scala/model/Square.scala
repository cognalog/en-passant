...
object Square {
  ...
  def fromStandardName(name: String): Try[Square] = name match {
    case square(file, rank) => Try(Square(file.toCharArray.head.toInt - 96, rank.toInt))
...
case class Square(file: Int, rank: Int) {
  /**
   * Get the square resulting from changing this square's file by the given amount.
   *
   * @param delta the amount (positive or negative) to increment the file.
   * @return the square resulting from the change.
   */
  ...
  /**
   * Get the square resulting from changing this square's rank by the given amount.
   *
   * @param delta the amount (positive or negative) to increment the rank.
   * @return the square resulting from the change.
   */
   ...
   *
   * @return the standard name for this square's file.
   */
  def standardFileName: String = s"${(file + 96).toChar}"

  /**
   * The standard name for this square, using the standard file name and the rank.
   *
   * @return the standard name for this square.
   */
