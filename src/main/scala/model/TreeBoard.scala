...
/** A board which does not process moves except as keys to a pre-set map of
  ...
  * @param turnColor
  *   color for the player whose turn it is.
  * @param children
  *   a map of possible next boards, keyed by corresponding moves. The
  *   move/board pairs don't need to make sense.
  */
case class TreeBoard(
    id: String,
    turnColor: Color,
    isCheckmate: Boolean = false,
    isDraw: Boolean = false,
    children: Map[Move, TreeBoard] = Map()
) extends Board {
