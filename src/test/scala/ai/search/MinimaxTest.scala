...
class MinimaxTest extends AnyFunSuite {
  ...
  test("testGetBestMove_wrongColor") {
    ...
    assertThrows[IllegalArgumentException] {
      minimax.GetBestMove(StandardBoard(Map(Square(1, 1) -> Pawn(Color.White)), turnColor = Color.Black), Color.White)
    }
  }
  ...
  test("testGetBestMove_depth0") {
    val minimax = Minimax(0, (_, _) => 5)
    assertResult(null) {
      minimax.GetBestMove(StandardBoard(Map(Square(1, 1) -> Pawn(Color.White))), Color.White)
    }
  }
